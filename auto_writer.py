...
[데이터 규칙]
1. 금리, 한도, 소득기준 등 구체적 숫자는 위 데이터에 있는 것만 사용.
2. 데이터에 없는 내용이라도 질문에 대한 답은 아는 지식으로 성실하게 답하세요. "모르겠다", "확인 필요" 같은 회피성 답변으로 글을 채우지 마세요.
3. 확실하지 않은 숫자만 "정확한 수치는 은행에 확인하세요" 한 줄로 처리. 글 전체를 "문의하세요"로 때우지 말 것.
...
2. HTML 태그 (<h2>, <h3>, <p>, <ul>, <li>, <table>) 사용
3. 구조:
   - 핵심 답변 (질문에 대한 직접적인 답을 먼저 1~2문장으로)
   - 왜 그런지 이유/근거 설명
   - 질문자가 실제로 해야 할 행동 안내 (구체적 절차, 연락처 등)
4. 1500~3000자
5. [절대 금지] 질문이 묻지 않은 내용은 쓰지 마세요:
   - "무주택자 인정되나요?" → 소득조건, 금리, LTV, DTI, 한도 쓰지 말 것
   - "퇴사하면 유지되나요?" → 자격조건표, 금리표 쓰지 말 것
   - "일시적 2주택 가능한가요?" → 소득기준, 대출한도, 주택가격 기준 쓰지 말 것
   - 질문의 답에 직접 필요한 정보만 쓰세요
6. [절대 금지] 글 전체를 "확인 필요", "문의하세요", "공식 채널에서 확인"으로 채우지 말 것. 답을 모르겠으면 아는 부분이라도 구체적으로 설명하고, 모르는 부분만 짧게 "은행에 확인하세요" 한 줄 처리.
7. 구어체 ("~해요", "~거든요") 사용하되 과하지 않게
8. 바로 핵심 답변부터 시작. 서론/인사 없이 첫 문장에서 답을 말할 것
9. 절대 금지 표현: "알아보겠습니다", "이는", "또한,", "아울러,", "결론적으로", "마무리하며", "도움이 되셨기를", "명시되어 있지 않습니다", "확인이 필요합니다"
10. <table>은 질문 답변에 꼭 필요할 때만. 불필요한 표/목록 나열 금지.
11. 실존하지 않는 제도/기관/URL 절대 만들지 말 것"""
