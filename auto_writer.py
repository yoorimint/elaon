...
[최우선 규칙 - 이것만 지키면 됨]
위 질문에 대한 답만 쓰세요. 질문이 묻지 않은 내용은 한 글자도 쓰지 마세요.
...
[참고 데이터 - 질문에 관련된 것만 사용]
{data_context}

[데이터 규칙]
- 구체적 숫자(금리/한도/소득)는 위 데이터에 있는 것만 사용.
- 데이터에 없어도 아는 지식으로 성실하게 답하세요.
- 모르는 건 언급하지 마세요.
...
===JSON_START===
{{
  "title": "질문을 자연스러운 제목으로 (40~60자, 연도 넣지 말 것)",
